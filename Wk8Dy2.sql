...
-- •What is the total number of employees? =36
SELECT COUNT(date_of_birth) FROM employee_detail;
...
-- JOIN employee_detail with current_job_detail i.e full join
SELECT name, job_title, salary
FROM employee_detail
JOIN current_job_detail ON employee_detail.employee_id = current_job_detail.employee_id;
...
-- PRIMARY KEYS
 CREATE TABLE music (id INT UNIQUE NOT NULL AUTO_INCREMENT PRIMARY KEY, title VARCHAR(225) NOT NULL);
 
-- FOREIGN KEYS (Define the column that can house the foreign key and have a reference for it)
 CREATE TABLE artists (id INT UNIQUE NOT NULL AUTO_INCREMENT PRIMARY KEY, name VARCHAR(255) NOT NULL, music_id INT, FOREIGN KEY (music_id) REFERENCES music(id));
